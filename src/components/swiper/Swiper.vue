    ...
    methods:{
        ...
        renderLazyDom(slideEls){
            slideEls.forEach((item,index)=>{
                ...
                if(item.className!="swiper-slide swiper-slide-selected"){
        ...
        updateEvent(pageSize){
            this.$nextTick(()=>{
                this.domTimer = setTimeout(()=>{
                    ...
                    this.swiperWrap = this.$el.querySelector('.swiper-wrap');
        ...
        _slideClassHandle(){
            let selectedSlide = this.$el.querySelector('.swiper-slide-selected');
            selectedSlide && selectedSlide.classList.remove('swiper-slide-selected');
            this.slideEls[this.currentPage -1].classList.add('swiper-slide-selected');
        ...
        _createLoop(){
            let propName = this.isHorizontal() ? 'offsetWidth' : 'offsetHeight';
            let swiperWrapEl = this.$el.querySelector('.swiper-wrap');
